...
static std::vector<char> QByteArrayToVectorC(const QByteArray byteArr)
{
    std::vector<char> vec;
    for (auto iter = byteArr.begin(); iter != byteArr.end(); ++iter)
        vec.push_back(*iter);
    return vec;
}

static QByteArray VectorCToQByteArray(const std::vector<char> &vec)
{
    QByteArray byteArr;
...
EdbCrypt::EdbCrypt(const QByteArray& key)
{
    std::vector<char> vec = QByteArrayToVectorC(key);
    SetKey(key.data(), key.size());
}


QByteArray EdbCrypt::Encrypt(const QByteArray& src) const
{
    return VectorCToQByteArray(Encrypt(QByteArrayToVectorC(src)));
}

QByteArray EdbCrypt::Decrypt(const QByteArray& src) const
{
    return VectorCToQByteArray(Decrypt(QByteArrayToVectorC(src)));
}


//EdbCrypt::EdbCrypt(const std::vector<char> &key) {
//  SetKey(key.data(), key.size());
//}

void EdbCrypt::SetKey(const char *key, size_t byte_length) {
...
std::vector<char> EdbCrypt::Encrypt(const std::vector<char> &src) const {
...
std::vector<char> EdbCrypt::Decrypt(const std::vector<char> &src) const {
  std::vector<char> dst = src;
...
void EdbCrypt::EncryptBlock(uint32_t *left, uint32_t *right) const {
  for (int i = 0; i < 16; ++i) {
    *left ^= pary_[i];
    *right ^= Feistel(*left);
    std::swap(*left, *right);
...
void EdbCrypt::DecryptBlock(uint32_t *left, uint32_t *right) const {
  for (int i = 0; i < 16; ++i) {
    *left ^= pary_[17 - i];
    *right ^= Feistel(*left);
    std::swap(*left, *right);
...
uint32_t EdbCrypt::Feistel(uint32_t value) const {
  Converter32 converter;
